...
|  | 出力されるメッセージに click が含まれないこと（終了後のステータスが0であること） | 19 | 条件指定をしないテスト実行で該当のprocが実行されないこと | 1. bucky run -d -D -pc -c pc_e2e_4<br/>- 以下シナリオファイル内の処理 -<br/>2. goメソッド実行 (http://bucky.net)<br/>2. whenを使ったclickメソッドは実行されない | 例外が発生していないこと（終了後のステータスが0であること）<br/>→ 「click」 が表示されないこと |
