...
@test "[linkstatus] #6 After executing linkstatus with excluding by regex, results have no failures nor errors" {
  run bucky run -t linkstatus -d -D pc -c pc_link_exclude_regex_1
  [ $status -eq 0 ]
  [ $(expr "$output" : ".*http://bucky\.net/test_page.html.*") -eq 0 ]
}

@test "[linkstatus] #7 After executing linkstatus with excluding by normal, results have no failures nor errors" {
