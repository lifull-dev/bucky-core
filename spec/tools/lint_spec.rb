...
describe Bucky::Tools::Lint do
  ...
  describe '#check_config' do
    it 'print error message, when there is difference of hash' do
      ...
      expect { lint.check_config }.to output("\e[31m[ERROR] The following configures are undefined. Tests can still be executed with default value automatically.\n- device_name_on_chrome-ipad\n{device_name_on_chrome-ipad: }\e[0m\n").to_stdout
    end
  ...
  describe '#make_message' do
    it 'when there are element in given array, print error message' do
      expect { lint.send(:make_message, ['foo']) }.to output("\e[31m[ERROR] The following configures are undefined. Tests can still be executed with default value automatically.\n- foo\n{foo: }\e[0m\n").to_stdout
    end
